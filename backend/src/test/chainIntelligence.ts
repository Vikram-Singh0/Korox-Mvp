...
async function testChainIntelligence() {
  ...
  try {
    ...
    allStats.forEach(stats => {
      ...
      console.log(
        `│ ${stats.chainName.padEnd(17)} │ ${health}     │ ${stats.currentBlock.toString().padEnd(12)} │ ${congestionEmoji} ${stats.networkCongestion.padEnd(6)} │ ${stats.estimatedGas.toFixed(4).padEnd(9)} │`
      );
    });
    ...
    for (const [source, dest] of routes) {
      try {
        ...
        console.log(
          `│ ${source.padEnd(12)} │ ${dest.padEnd(12)} │ ${(feeEstimate.estimatedFee + ' ' + feeEstimate.currency).padEnd(12)} │ ${confidenceEmoji} ${feeEstimate.confidence.padEnd(6)} │`
        );
      } catch (error) {
