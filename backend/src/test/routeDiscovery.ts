...
async function testRouteDiscovery() {
  ...
  try {
    ...
    const testRoutes: Array<[ChainName, ChainName]> = [
      ['assetHub', 'moonbeam'],
      ['assetHub', 'astar'],
      ['moonbeam', 'astar']
    ];
    ...
    for (const [source, dest] of testRoutes) {
      ...
      const priorities: OptimizationPriority[] = ['balanced', 'fastest', 'cheapest'];
    ...
    comparisonRoutes.forEach((route, index) => {
      if (route) {
        ...
        console.log(
          `│ ${name.padEnd(10)} │ ${route.score.toString().padEnd(8)} │ ${route.breakdown.gasCost.toString().padEnd(12)} │ ${route.breakdown.estimatedTime.toString().padEnd(8)} │ ${route.breakdown.reliability.toString().padEnd(12)}% │`
        );
      }
    });
    ...
    const checkRoutes: Array<[ChainName, ChainName]> = [
      ['assetHub', 'moonbeam'],
      ['assetHub', 'astar'],
      ['moonbeam', 'astar']
    ];
    ...
    for (const [from, to] of checkRoutes) {
      for (const asset of assets) {
        ...
        console.log(
          `│ ${(from + '→' + to).padEnd(14)} │ ${asset.padEnd(16)} │ ${supportedStr.padEnd(23)} │`
        );
      }
    }
