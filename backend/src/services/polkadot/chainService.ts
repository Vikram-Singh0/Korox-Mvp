...
export interface ChainStats {
  ...
  estimatedGas: number; // in chain's native token
  lastUpdated: number; // timestamp
}

export interface CongestionAnalysis {
  blockFullness: number; // percentage 0-100
...
export class ChainIntelligenceService {
  private apiClient: PolkadotApiClient;
   ...
   */
  async getChainStats(chainName: ChainName): Promise<ChainStats> {
    const cacheKey = `stats:${chainName}`;

    // Check cache first
    const cached = this.cache.get<ChainStats>(cacheKey);
    if (cached) {
  ...
  async analyzeCongestion(
    ...
    try {
      ...
      const congestionScore = Math.round(
        blockFullness * 0.6 + queueScore * 0.4
      );
      ...
      logger.debug(
        `${chainName} congestion: ${congestionLevel} (score: ${congestionScore}, block: ${blockFullness.toFixed(
          1
        )}%, queue: ${transactionQueueSize})`
      );
      ...
      logger.warn(
        `Congestion analysis failed for ${chainName}, using defaults:`,
        error
      );
   ...
   */
  async estimateGas(api: ApiPromise, chainName: ChainName): Promise<number> {
    try {
  ...
  async estimateXcmFee(
    ...
    destChain: ChainName,
    amount?: string
  ): Promise<XcmFeeEstimate> {
    const cacheKey = `xcm:${sourceChain}:${destChain}`;

    const cached = this.cache.get<XcmFeeEstimate>(cacheKey);
    if (cached) {
      return cached;
    }
    ...
    try {
      ...
      if (sourceStats.congestionScore > 70 || destStats.congestionScore > 70) {
        ...
        sourceStats.congestionScore > 40 ||
      ...
      const totalFee =
        (baseXcmFee + sourceGas + destExecutionCost) * congestionMultiplier;
      ...
      if (!sourceStats.isHealthy || !destStats.isHealthy) {
        ...
        sourceStats.congestionScore > 50 ||
        destStats.congestionScore > 50
      ) {
        confidence = "medium";
      }

      const estimate: XcmFeeEstimate = {
        sourceChain,
        destChain,
      ...
      logger.debug(
        `XCM fee estimate: ${sourceChain} → ${destChain} = ${estimate.estimatedFee} ${estimate.currency} (${confidence} confidence)`
      );

      return estimate;
    } catch (error) {
      logger.error(
        `XCM fee estimation failed for ${sourceChain} → ${destChain}:`,
        error
      );
   ...
   */
  async getAllChainStats(): Promise<ChainStats[]> {
    ...
    const statsPromises = chains.map((chain) =>
      this.getChainStats(chain).catch((error) => {
    ...
    return results
      ...
      .map((result) => (result as PromiseFulfilledResult<ChainStats>).value);
  }
