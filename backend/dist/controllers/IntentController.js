...
export async function getSupportedChains(req, res) {
    try {
        ...
        const chainStatuses = await Promise.all(allChains.map(async (chain) => {
            try {
 ...
 */
export async function solveIntent(req, res) {
    try {
        const { fromChain, toChain, token, amount, toAddress, prefrences } = req.body;
        // Validation
        ...
        const allRoutes = await routeOptimizer.compareRoutes(fromChain, toChain, token);
 ...
 */
function buildRouteResponse(route, fromChain, toChain, token, amount) {
    // Build steps from path
    const steps = route.path.map((chain, index) => ({
        chain,
