...
async function testChainIntelligence() {
    ...
    try {
        ...
        console.log('├─────────────────────────────────────────────────────────┤');
        console.log(`│  Health:      ${assetHubStats.isHealthy ? '✅ Healthy' : '❌ Unhealthy'}                                │`);
        console.log(`│  Block:       #${assetHubStats.currentBlock.toString().padEnd(40)} │`);
        console.log(`│  Block Time:  ${assetHubStats.avgBlockTime}s avg                                   │`);
        console.log(`│  Congestion:  ${assetHubStats.networkCongestion.toUpperCase().padEnd(6)} (score: ${assetHubStats.congestionScore})            │`);
        console.log(`│  Est. Gas:    ${assetHubStats.estimatedGas} DOT                               │`);
        console.log('└─────────────────────────────────────────────────────────┘\n');
        // Test 2: Test caching
        ...
        console.log('\n┌───────────────────┬─────────┬──────────────┬─────────────┬───────────┐');
        console.log('│ Chain             │ Health  │ Block        │ Congestion  │ Est. Gas  │');
        console.log('├───────────────────┼─────────┼──────────────┼─────────────┼───────────┤');
        allStats.forEach(stats => {
            const health = stats.isHealthy ? '✅' : '❌';
            const congestionEmoji = stats.networkCongestion === 'low' ? '🟢' :
                stats.networkCongestion === 'medium' ? '🟡' : '🔴';
            console.log(`│ ${stats.chainName.padEnd(17)} │ ${health}     │ ${stats.currentBlock.toString().padEnd(12)} │ ${congestionEmoji} ${stats.networkCongestion.padEnd(6)} │ ${stats.estimatedGas.toFixed(4).padEnd(9)} │`);
        });
        console.log('└───────────────────┴─────────┴──────────────┴─────────────┴───────────┘\n');
        logger.success(`Successfully retrieved stats for ${allStats.length} chains`);
        ...
        for (const [source, dest] of routes) {
            try {
                const feeEstimate = await chainIntelligence.estimateXcmFee(source, dest);
                const confidenceEmoji = feeEstimate.confidence === 'high' ? '🟢' :
                    feeEstimate.confidence === 'medium' ? '🟡' : '🔴';
                console.log(`│ ${source.padEnd(12)} │ ${dest.padEnd(12)} │ ${(feeEstimate.estimatedFee + ' ' + feeEstimate.currency).padEnd(12)} │ ${confidenceEmoji} ${feeEstimate.confidence.padEnd(6)} │`);
            }
            catch (error) {
                console.log(`│ ${source.padEnd(12)} │ ${dest.padEnd(12)} │ ERROR        │ ❌ N/A    │`);
            }
        }
        console.log('└──────────────┴──────────────┴──────────────┴────────────┘\n');
        ...
        console.log('╠═══════════════════════════════════════════════════════════╣');
        console.log('║  ✅ Single chain stats retrieval                          ║');
        console.log('║  ✅ Caching system working                                ║');
        console.log(`║  ✅ Multi-chain stats (${allStats.length}/8 chains)                      ║`);
        console.log('║  ✅ XCM fee estimation                                    ║');
        console.log('║  ✅ Congestion analysis                                   ║');
        console.log('╚═══════════════════════════════════════════════════════════╝\n');
    }
    catch (error) {
